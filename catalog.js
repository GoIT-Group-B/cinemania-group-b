import{d as J,f as w,E as u,B as T,c as W,I as N,b as ae,a as re,s as q}from"./assets/footer-KgHnxcCY.js";import"./assets/vendor-DDD7fsZd.js";const ie={trendCard:"trend-card",posterWrapper:"poster-wrapper",moviePoster:"movie-poster",movieMeta:"movie-meta",trendInfo:"trend-info",trendTitle:"trend-title",movieDetails:"movie-details",trendStars:"trend-stars",movieRating:"movie-rating"},D=document.getElementById("movieList");let F=1,U=20;async function R(e=1){const t=await w(T,u.POPULAR_MOVIES,{page:e}),n=await t.results;F=e,U=t.total_pages,oe(n),K(F,U,"first")}function oe(e){const{trendCard:t,posterWrapper:n,moviePoster:s,movieMeta:r,trendInfo:i,trendTitle:l,movieDetails:I,trendStars:L,movieRating:C}=ie;D.innerHTML=e.map(v=>{const{title:o,poster_path:f,release_date:d,vote_average:m,genre_ids:g}=v,_=d?d.split("-")[0]:"N/A",$=g.slice(0,2).map(h=>le[h]).join(", "),B=W(m);return`
      ...
      `}).join("");K(V,Y,"search"),E.innerHTML=_,E.scrollIntoView({behavior:"smooth"})}catch(m){console.error("Error fetching movies:",m),P.style.display="block"}}p.addEventListener("input",()=>{y.style.display=p.value.trim()!==""?"block":"none",p.value.trim()!==""||y.value!==""?b.style.display="block":b.style.display="none",p.value.trim()===""&&Q()});y.addEventListener("change",()=>{p.value.trim()!==""||y.value!==""?(b.style.display="block",S()):b.style.display="none"});function Q(){p.value="",y.value="",b.style.display="none",y.style.display="none",P.style.display="none",E.innerHTML="",H&&(H.style.display="grid"),p.focus()}b.addEventListener("click",Q);ce.addEventListener("click",async()=>{await S()});document.addEventListener("DOMContentLoaded",()=>{E&&J(E)});const ve={container:".hero-container",content:"hero-content",title:"hero-title",rating:"hero-rating",description:"hero-description",buttons:"hero-buttons",trailerBtn:"watch-trailer-btn",detailsBtn:"more-details-btn",trailerBtnClass:".watch-trailer-btn",detailsBtnClass:".more-details-btn"},G={trailerModal:"trailerModal",trailerIframe:"trailerIframe",trailerCloseBtn:"closeTrailer"};async function pe(){const t=await(await w(T,u.POPULAR_MOVIES)).results;let n=0;const s=document.querySelector(".hero-container");z(t[n]),setInterval(()=>{s.style.opacity="0",setTimeout(()=>{n=(n+1)%t.length,z(t[n]),s.style.opacity="1"},500)},5e3)}function z(e){const{container:t,content:n,title:s,rating:r,description:i,buttons:l,trailerBtn:I,detailsBtn:L,trailerBtnClass:C,detailsBtnClass:v}=ve,o=document.querySelector(t),d=window.innerWidth<480?`${u.IMG_W500}${e.poster_path}`:`${u.IMG_W1280}${e.backdrop_path}`,m=`${N}${d}`;o.style.backgroundImage=`url('${m}')`,o.style.backgroundSize="cover",o.style.backgroundPosition="center",o.style.backgroundRepeat="no-repeat";const g=W(e.vote_average);o.innerHTML=`
    <div class="${n}">
  ...
  `,document.querySelector(C).addEventListener("click",()=>he(e.id)),document.querySelector(v).addEventListener("click",()=>ae(e))}async function ye(){const e=await w(T,u.GENRE_LIST);ge(e.genres)}function ge(e){const t=document.getElementById("category");t&&(t.innerHTML=e.map(n=>`
        <div class="${categoryCard}">${n.name}</div>
      `).join(""))}pe();ye();async function he(e){try{const n=(await w(T,u.MOVIE_VIDEOS(e))).results.find(s=>s.type==="Trailer"&&s.site==="YouTube");n?re(n.key):q()}catch(t){console.error("Trailer fetch error:",t),q()}}document.getElementById(G.trailerCloseBtn).addEventListener("click",()=>{const{trailerModal:e,trailerIframe:t}=G,n=document.getElementById(e),s=document.getElementById(t);n.style.display="none",s.src=""});
