import"./assets/theme-Df9oKh5u.js";import{I as l,E as i,f as u,B as c,c as y}from"./assets/stars-ZdAbMgea.js";import"./assets/vendor-DDD7fsZd.js";const p=document.getElementById("movieDetailModal"),d=document.getElementById("detailPoster"),f=document.getElementById("detailTitle"),M=document.getElementById("detailVotes"),E=document.getElementById("detailPopularity"),$=document.getElementById("detailGenres"),w=document.getElementById("detailOverview"),B=document.getElementById("closeDetailModal");function b(e,t){try{d.src=`${l}${i.IMG_W500}${e.poster_path}`,d.alt=e.title,f.textContent=e.title,M.textContent=`${e.vote_average} / ${e.vote_count}`,E.textContent=Math.floor(e.popularity);const n=e.genre_ids||(e.genres?e.genres.map(o=>o.id):[]);(!t||Object.keys(t).length===0)&&(console.warn("Genre map boş veya tanımsız! Genre verileri henüz yüklenmemiş."),$.textContent="Unknown"),w.textContent=e.overview||"No description available.",p.classList.add("active")}catch(n){console.error("openMovieDetailModal hatası:",n)}}B.addEventListener("click",()=>{p.classList.remove("active")});async function _(){const t=await(await u(c,i.POPULAR_MOVIES)).results;let n=0;const o=document.querySelector(".hero-container");m(t[n]),setInterval(()=>{o.style.opacity="0",setTimeout(()=>{n=(n+1)%t.length,m(t[n]),o.style.opacity="1"},500)},5e3)}function m(e){const t=document.querySelector(".hero-container"),n=`${l}${i.IMG_W1280}${e.backdrop_path}`;t.style.backgroundImage=`url('${n}')`;const o=y(e.vote_average);t.innerHTML=`
  ...
  `,document.querySelector(".watch-trailer-btn").addEventListener("click",()=>T(e.id)),document.querySelector(".more-details-btn").addEventListener("click",()=>b(e))}async function L(){const t=await(await fetch(`${c}/genre/movie/list?api_key=${API_KEY}&language=en-US`)).json();S(t.genres)}function S(e){const t=document.getElementById("category");t&&(t.innerHTML=e.map(n=>`
      ...
      `).join(""))}_();L();async function T(e){const o=(await(await fetch(`${c}/movie/${e}/videos?api_key=52238d7fab5c2c01b99e751619dd16ec&language=en-US`)).json()).results.find(r=>r.type==="Trailer"&&r.site==="YouTube"),a=document.getElementById("trailerIframe"),s=document.getElementById("trailerModal");o?(a.src=`https://www.youtube.com/embed/${o.key}`,s.style.display="block"):alert("Trailer bulunamadı.")}document.getElementById("closeTrailer").addEventListener("click",()=>{const e=document.getElementById("trailerModal"),t=document.getElementById("trailerIframe");e.style.display="none",t.src=""});const k=document.getElementById("movieList");async function C(){const t=await(await u(c,i.POPULAR_MOVIES)).results;console.log(t),A(t)}function A(e){k.innerHTML=e.map(t=>{const{title:n,poster_path:o,release_date:a,vote_average:s,genre_ids:r}=t,v=a?a.split("-")[0]:"N/A",g=r.slice(0,2).map(I=>P[I]).join(", "),h=y(s);return`
