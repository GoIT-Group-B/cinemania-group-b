import{a as S}from"./vendor-DDD7fsZd.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))c(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const a of r.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&c(a)}).observe(document,{childList:!0,subtree:!0});function o(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function c(n){if(n.ep)return;n.ep=!0;const r=o(n);fetch(n.href,r)}})();const A={menuToggleClass:".menu-toggle",mobileMenuId:"mobileMenu",mobileOverlayId:"mobileOverlay"},{menuToggleClass:G,mobileMenuId:C,mobileOverlayId:N}=A,E=document.querySelector(G),b=document.getElementById(C),M=document.getElementById(N);E.addEventListener("click",()=>{b.classList.toggle("open"),M.classList.toggle("active")});document.addEventListener("click",e=>{const t=b.contains(e.target),o=E.contains(e.target);!t&&!o&&(b.classList.remove("open"),M.classList.remove("active"))});const x="modulepreload",F=function(e){return"/cinemania-group-b/"+e},_={},$=function(t,o,c){let n=Promise.resolve();if(o&&o.length>0){let a=function(l){return Promise.all(l.map(i=>Promise.resolve(i).then(u=>({status:"fulfilled",value:u}),u=>({status:"rejected",reason:u}))))};document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),d=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));n=a(o.map(l=>{if(l=F(l),l in _)return;_[l]=!0;const i=l.endsWith(".css"),u=i?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${u}`))return;const m=document.createElement("link");if(m.rel=i?"stylesheet":x,i||(m.as="script"),m.crossOrigin="",m.href=l,d&&m.setAttribute("nonce",d),document.head.appendChild(m),i)return new Promise((h,P)=>{m.addEventListener("load",h),m.addEventListener("error",()=>P(new Error(`Unable to preload CSS for ${l}`)))})}))}function r(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return n.then(a=>{for(const s of a||[])s.status==="rejected"&&r(s.reason);return t().catch(r)})},p=`<svg class="star" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<stop offset="1" stop-color="#F89F19" stop-opacity="0.68"/>
...
</svg>`;function H(e){let t="";if(!e)return t=`${p.repeat(5)}`,`<div>${t}</div>`;switch(Math.round(e)){case 0:t=`${p.repeat(5)}`;break;case 1:t=`${y}${p.repeat(4)}`;break;case 2:t=`${v}${p.repeat(4)}`;break;case 3:t=`${v}${y}${p.repeat(3)}`;break;case 4:t=`${v.repeat(2)}${p.repeat(3)}`;break;case 5:t=`${v.repeat(2)}${y}${p.repeat(2)}`;break;case 6:t=`${v.repeat(3)}${p.repeat(2)}`;break;case 7:t=`${v.repeat(3)}${y}${p}`;break;case 8:t=`${v.repeat(4)}${p}`;break;case 9:t=`${v.repeat(4)}${y}`;break;case 10:t=`${v.repeat(5)}`;break;default:throw new Error("Invalid rating")}return`<div>${t}</div>`}const k="52238d7fab5c2c01b99e751619dd16ec",I="https://api.themoviedb.org/3",R="https://image.tmdb.org/t/p",O={POPULAR_MOVIES:"/movie/popular",UPCOMING_MOVIES:"/movie/upcoming",TRENDING_WEEK:"/trending/movie/week",TRENDING_DAY:"/trending/movie/day",SEARCH_MOVIES:"/search/movie",GENRE_LIST:"/genre/movie/list",MOVIE_DETAILS:e=>`/movie/${e}`,MOVIE_VIDEOS:e=>`/movie/${e}/videos`,IMG_ORIGINAL:"/original",IMG_W500:"/w500",IMG_W780:"/w780",IMG_W1280:"/w1280"};async function B(e,t,o={}){try{return(await S.get(`${e}${t}`,{params:{api_key:k,language:"en-US",page:1,...o}})).data}catch(c){throw console.error("Error fetching data:",c),c}}async function Q(){try{const e=await S.get(`${I}${O.GENRE_LIST}`,{params:{api_key:k,language:"en-US"}}),t={};return e.data.genres.forEach(o=>{t[o.id]=o.name}),t}catch(e){return console.error("Error fetching genres:",e),{}}}function U({movies:e,renderCallback:t,containerSelector:o,btnSelector:c,moviesPerPage:n=9}){let r=1;const a=document.querySelector(c),s=document.querySelector(o);if(!a||!s)return;function d(){const i=r*n,u=e.slice(0,i);s.innerHTML="",t(u),i>=e.length?a.style.display="none":a.style.display="block"}d(),a.addEventListener("click",()=>{r++,d()})}const f={28:"Action",12:"Adventure",16:"Animation",35:"Comedy",80:"Crime",99:"Documentary",18:"Drama",10751:"Family",14:"Fantasy",36:"History",27:"Horror",10402:"Music",9648:"Mystery",10749:"Romance",878:"Science Fiction",10770:"TV Movie",53:"Thriller",10752:"War",37:"Western"},j=Object.entries(f).reduce((e,[t,o])=>(e[o.toLowerCase()]=parseInt(t),e),{});function D(){const e=document.querySelector(".library-gallery .dropdown select");e&&(e.innerHTML=`
    <option class="hidden-option" value="Genre" selected disabled>Genre</option>
    <option value="all">All</option>
  `,Object.values(f).sort().forEach(t=>{const o=document.createElement("option");o.value=t.toLowerCase(),o.textContent=t,e.appendChild(o)}))}function L(e=null){const t=document.querySelector(".library-gallery"),o=t==null?void 0:t.querySelector(".library-gallery-list");if(!t||!o){window.location.pathname.endsWith("library.html")&&console.warn("library-gallery veya library-gallery-list bulunamadı.");return}const c=JSON.parse(localStorage.getItem("myLibrary"))||[];let n=c;if(e&&e!=="Genre"&&e!=="all"){const a=j[e];a&&(n=c.filter(s=>{var d;return(d=s.genre_ids)==null?void 0:d.includes(a)}))}if(o.innerHTML="",n.length===0){const a=`
    ...
    `),D(),r=t.querySelector(".dropdown select")),e&&r.options.length>1&&(r.value=e),U({movies:n,renderCallback:q,containerSelector:".library-gallery-list",btnSelector:".load-more-btn",moviesPerPage:9}),r==null||r.addEventListener("change",a=>{const s=a.target.value;L(s)})}function T(e){e&&e.addEventListener("click",async t=>{var d,l;const o=t.target.closest(".trend-card");if(!o)return;const c=o.dataset.id;let r=(JSON.parse(localStorage.getItem("myLibrary"))||[]).find(i=>i.id===Number(c));if(!r)try{r=await B(I,O.MOVIE_DETAILS(c)),r.genre_ids=((d=r.genres)==null?void 0:d.map(i=>i.id))||[]}catch(i){console.error("API fetch error:",i);return}const{showDetailsModal:a}=await $(async()=>{const{showDetailsModal:i}=await Promise.resolve().then(()=>z);return{showDetailsModal:i}},void 0),s=((l=r.genre_ids)==null?void 0:l.map(i=>f[i]).filter(Boolean))||[];a(r,s)})}function q(e){const t=e.map(c=>{const{id:n,title:r,poster_path:a,release_date:s,vote_average:d,genre_ids:l}=c,i=s?s.split("-")[0]:"N/A",u=(l==null?void 0:l.map(h=>f[h]).filter(Boolean).join(", "))||"",m=H(d);return`
        <div class="trend-card" data-id="${n}">
          <div class="poster-wrapper">
            <img src="${R}/w500${a}" alt="${r}" class="movie-poster" />
  ...
  `)}function Z(e,t=[]){var s,d;const c=(JSON.parse(localStorage.getItem("myLibrary"))||[]).some(l=>l.id===e.id),n=c?"Remove from Library":"Add to My Library",r=t.length>0?t.join(" "):((s=e.genre_ids)==null?void 0:s.map(l=>W[l]).filter(Boolean).join(" "))||"",a=`
  ...
  `;w(a),(d=document.querySelector(".add-to-library-btn"))==null||d.addEventListener("click",async()=>{let l=JSON.parse(localStorage.getItem("myLibrary"))||[];c?(l=l.filter(i=>i.id!==e.id),alert("Film kütüphaneden kaldırıldı.")):(l.push(e),alert("Film kütüphaneye eklendi.")),localStorage.setItem("myLibrary",JSON.stringify(l)),g();try{const{loadLibrary:i}=await $(async()=>{const{loadLibrary:u}=await Promise.resolve().then(()=>V);return{loadLibrary:u}},void 0);i&&i()}catch(i){console.error("loadLibrary yüklenirken hata oluştu:",i)}})}const z=Object.freeze(Object.defineProperty({__proto__:null,showDetailsModal:Z,showErrorModal:K,showTrailerModal:J},Symbol.toStringTag,{value:"Module"}));window.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("theme-switcher"),t=document.documentElement;localStorage.getItem("theme")==="light"?t.classList.add("light-theme"):t.classList.remove("light-theme"),e.addEventListener("click",()=>{const c=t.classList.toggle("light-theme");localStorage.setItem("theme",c?"light":"dark")})});export{I as B,O as E,R as I,J as a,Z as b,H as c,T as d,Q as e,B as f,K as s};
//# sourceMappingURL=theme-LYYpEH5U.js.map
