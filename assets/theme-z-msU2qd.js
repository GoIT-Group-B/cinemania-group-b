...
</svg>`;function R(e){let t="";if(!e)return t=`${p.repeat(5)}`,`<div>${t}</div>`;switch(Math.round(e)){case 0:t=`${p.repeat(5)}`;break;case 1:t=`${y}${p.repeat(4)}`;break;case 2:t=`${v}${p.repeat(4)}`;break;case 3:t=`${v}${y}${p.repeat(3)}`;break;case 4:t=`${v.repeat(2)}${p.repeat(3)}`;break;case 5:t=`${v.repeat(2)}${y}${p.repeat(2)}`;break;case 6:t=`${v.repeat(3)}${p.repeat(2)}`;break;case 7:t=`${v.repeat(3)}${y}${p}`;break;case 8:t=`${v.repeat(4)}${p}`;break;case 9:t=`${v.repeat(4)}${y}`;break;case 10:t=`${v.repeat(5)}`;break;default:throw new Error("Invalid rating")}return`<div>${t}</div>`}const I="52238d7fab5c2c01b99e751619dd16ec",k="https://api.themoviedb.org/3",B="https://image.tmdb.org/t/p",O={POPULAR_MOVIES:"/movie/popular",UPCOMING_MOVIES:"/movie/upcoming",TRENDING_WEEK:"/trending/movie/week",TRENDING_DAY:"/trending/movie/day",SEARCH_MOVIES:"/search/movie",DISCOVER_MOVIES:"/discover/movie",GENRE_LIST:"/genre/movie/list",MOVIE_DETAILS:e=>`/movie/${e}`,MOVIE_VIDEOS:e=>`/movie/${e}/videos`,IMG_ORIGINAL:"/original",IMG_W500:"/w500",IMG_W780:"/w780",IMG_W1280:"/w1280"};async function D(e,t,o={}){try{return(await S.get(`${e}${t}`,{params:{api_key:I,language:"en-US",page:1,...o}})).data}catch(c){throw console.error("Error fetching data:",c),c}}async function Q(){try{const e=await S.get(`${k}${O.GENRE_LIST}`,{params:{api_key:I,language:"en-US"}}),t={};return e.data.genres.forEach(o=>{t[o.id]=o.name}),t}catch(e){return console.error("Error fetching genres:",e),{}}}function H({movies:e,renderCallback:t,containerSelector:o,btnSelector:c,moviesPerPage:n=9}){let r=1;const a=document.querySelector(c),l=document.querySelector(o);if(!a||!l)return;function d(){const s=(r-1)*n,i=r*n,u=e.slice(s,i);t(u),i>=e.length?a.style.display="none":a.style.display="block"}d(),a.addEventListener("click",()=>{r++,d()})}const f={28:"Action",12:"Adventure",16:"Animation",35:"Comedy",80:"Crime",99:"Documentary",18:"Drama",10751:"Family",14:"Fantasy",36:"History",27:"Horror",10402:"Music",9648:"Mystery",10749:"Romance",878:"Science Fiction",10770:"TV Movie",53:"Thriller",10752:"War",37:"Western"},U=Object.entries(f).reduce((e,[t,o])=>(e[o.toLowerCase()]=parseInt(t),e),{});function j(){const e=document.querySelector(".library-gallery .dropdown select");e&&(e.innerHTML=`
    <option class="hidden-option" value="Genre" selected disabled>Genre</option>
    <option value="all">All</option>
  `,Object.values(f).sort().forEach(t=>{const o=document.createElement("option");o.value=t.toLowerCase(),o.textContent=t,e.appendChild(o)}))}function L(e=null){const t=document.querySelector(".library-gallery"),o=t==null?void 0:t.querySelector(".library-gallery-list");if(!t||!o){window.location.pathname.endsWith("library.html")&&console.warn("library-gallery veya library-gallery-list bulunamadı.");return}const c=JSON.parse(localStorage.getItem("myLibrary"))||[];let n=c;if(e&&e!=="Genre"&&e!=="all"){const a=U[e];a&&(n=c.filter(l=>{var d;return(d=l.genre_ids)==null?void 0:d.includes(a)}))}if(o.innerHTML="",n.length===0){const a=`
    ...
    `),j(),r=t.querySelector(".dropdown select")),e&&r.options.length>1&&(r.value=e),H({movies:n,renderCallback:V,containerSelector:".library-gallery-list",btnSelector:".load-more-btn",moviesPerPage:9}),r==null||r.addEventListener("change",a=>{const l=a.target.value;L(l)})}function T(e){e&&e.addEventListener("click",async t=>{var d,s;const o=t.target.closest(".trend-card");if(!o)return;const c=o.dataset.id;let r=(JSON.parse(localStorage.getItem("myLibrary"))||[]).find(i=>i.id===Number(c));if(!r)try{r=await D(k,O.MOVIE_DETAILS(c)),r.genre_ids=((d=r.genres)==null?void 0:d.map(i=>i.id))||[]}catch(i){console.error("API fetch error:",i);return}const{showDetailsModal:a}=await $(async()=>{const{showDetailsModal:i}=await Promise.resolve().then(()=>z);return{showDetailsModal:i}},void 0),l=((s=r.genre_ids)==null?void 0:s.map(i=>f[i]).filter(Boolean))||[];a(r,l)})}function V(e){const t=e.map(c=>{const{id:n,title:r,poster_path:a,release_date:l,vote_average:d,genre_ids:s}=c,i=l?l.split("-")[0]:"N/A",u=(s==null?void 0:s.map(h=>f[h]).filter(Boolean).join(", "))||"",m=R(d);return`
